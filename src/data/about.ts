export const about = {
  name: 'Lara Betina Koeppe da Silva',
  shortName: 'Lara Koeppe',
  title: 'Arquiteta e Urbanista',
  location: 'Santa Cruz do Sul, RS',
  bio: [
    'Bacharelado em Arquitetura e Urbanismo pela UNISC, com atuação no segmento da Arquitetura e construção civil, contribuindo no desenvolvimento de projetos nas diversas escalas, de detalhamento à finalização.',
    'Experiência em modelagem 3D, renderização, detalhamento técnico e design de interiores, utilizando softwares como AutoCAD, SketchUp e Luminio para dar vida a cada projeto.',
  ],
  education: [
    {
      degree: 'Arquitetura e Urbanismo',
      institution: 'UNISC',
    },
    {
      degree: 'Ensino Fundamental e Médio',
      institution: 'E.E.E.M. Ernesto Alves de Oliveira',
    },
  ],
  languages: [
    { name: 'Português', level: 'Nativo' },
    { name: 'Inglês', level: 'Avançado' },
    { name: 'Alemão', level: 'Compreensão' },
  ],
  software: [
    'AutoCAD',
    'SketchUp',
    'Lumion',
    'Enscape',
    'Promob',
    'Illustrator',
    'Photoshop',
    'Pacote Office',
  ],
  experience: [
    {
      role: 'Estagiária de Arquitetura',
      company: 'Ronaldo Wink e Nathan Della Nina',
    },
    {
      role: 'Vendedora Projetista',
      company: 'Tendência Móveis Projetados (FINGER)',
    },
    {
      role: 'Assistente Administrativo',
      company: 'Condomínio Belle Ville',
    },
  ],
  experienceStartYear: 2017,
  stats: [
    { value: `${new Date().getFullYear() - 2017}`, label: 'Anos de Experiência' },
    { value: '8', label: 'Softwares Dominados' },
    { value: '3', label: 'Experiências Profissionais' },
  ],
}
